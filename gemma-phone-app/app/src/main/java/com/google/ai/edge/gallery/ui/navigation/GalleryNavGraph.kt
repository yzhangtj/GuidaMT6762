...
fun GalleryNavHost(
  ...
  LaunchedEffect(modelManagerUiState.tasks, autoNavigated) {
    if (!autoNavigated) {
      val askImageTask =
        modelManagerUiState.tasks.firstOrNull { it.id == BuiltInTaskId.LLM_ASK_IMAGE }
      val defaultModel = askImageTask?.models?.firstOrNull()
      if (askImageTask != null && defaultModel != null) {
        autoNavigated = true
        pickedTask = askImageTask
        showModelManager = true
        modelManagerViewModel.selectModel(defaultModel)
        navController.navigate("$ROUTE_MODEL/${askImageTask.id}/${defaultModel.name}")
      }
    }
  }
